...
- **奖励模式**：环境支持三种奖励模式：
  ...
  - `protect`：用于训练 **Protect 技能** - 无时间惩罚，有防御者到目标边界的距离引导奖励（权重=8），超时计为防御者胜利 (+20)
...
- **奖励结构**：取决于模式，但一般为：
  ...
  - Attacker 到达 Target：-20（Attacker 碰撞障碍物不结束 episode）
  - 额外的模式特定奖励/惩罚（见上文）
...
1. **Protect 技能**：
   ...
   - **奖励模式**：`protect`
   - **GRU**：不使用（防御者知道目标位置）
   - **奖励**：防御者到目标边界距离引导（权重=8），胜利+20，失败-20，无时间惩罚
...
**双技能方法**：
- **Protect**：学习将防御者定位在攻击者和目标之间，使用到目标边界的距离作为引导（权重=8）
