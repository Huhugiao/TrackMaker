...
class ObstacleDensity:
    ...
    CLUTTERED = "cluttered"
    SCATTERED = "scattered"
    ALL_LEVELS = [NONE, DENSE, CLUTTERED, SCATTERED]
...
# Obstacle randomization (enabled by default for both training and evaluation).
_obstacle_randomization_enabled = False
_jitter_px = 36
_jitter_seed = None
...
_SCATTERED_INDICES = [0, 1, 2, 4, 6, 7, 8, 10, 12, 13, 14, 16, 17, 18, 20, 22, 23, 24]
_SCATTERED_OBSTACLES = [_DENSE_OBSTACLES[idx] for idx in _SCATTERED_INDICES]

_CLUTTERED_EXTRA_OBSTACLES = [
    {'type': 'circle', 'cx': 125, 'cy': 120, 'r': 14, 'color': OBSTACLE_COLOR},
    {'type': 'rect', 'x': 240, 'y': 115, 'w': 28, 'h': 24, 'color': OBSTACLE_COLOR},
    {'type': 'circle', 'cx': 515, 'cy': 120, 'r': 14, 'color': OBSTACLE_COLOR},
    {'type': 'segment', 'x1': 125, 'y1': 260, 'x2': 155, 'y2': 260, 'thick': 8, 'color': OBSTACLE_COLOR},
    {'type': 'segment', 'x1': 485, 'y1': 260, 'x2': 515, 'y2': 260, 'thick': 8, 'color': OBSTACLE_COLOR},
    {'type': 'circle', 'cx': 125, 'cy': 520, 'r': 14, 'color': OBSTACLE_COLOR},
    {'type': 'rect', 'x': 240, 'y': 500, 'w': 28, 'h': 24, 'color': OBSTACLE_COLOR},
    {'type': 'circle', 'cx': 515, 'cy': 520, 'r': 14, 'color': OBSTACLE_COLOR},
]

OBSTACLE_DENSITY_SPECS = {
    ...
    ObstacleDensity.CLUTTERED: {
        'count_scale': 1.32,
        'jitter_px': 15,
        'randomization_enabled': True,
        'display_name': 'cluttered',
    },
    ObstacleDensity.SCATTERED: {
        'count_scale': 0.72,
        'jitter_px': 25,
        'randomization_enabled': True,
...
OBSTACLE_PRESETS = {
    ...
    ObstacleDensity.CLUTTERED: list(MAP_LAYOUT_PRESETS.get(DEFAULT_MAP_LAYOUT, _DENSE_OBSTACLES)) + list(_CLUTTERED_EXTRA_OBSTACLES),
    ObstacleDensity.SCATTERED: list(_SCATTERED_OBSTACLES),
}
...
def _jitter_obstacle(obs, rng, jitter_px):
    """Apply a bounded random translation to one obstacle."""
    item = dict(obs)
    if jitter_px <= 0:
        return item
    ...
    if item['type'] == 'rect':
        w = float(item['w'])
        h = float(item['h'])
        x = _clamp(float(item['x']) + dx, wall, float(width) - wall - w)
        y = _clamp(float(item['y']) + dy, wall, float(height) - wall - h)
        item['x'] = float(x)
        item['y'] = float(y)
    elif item['type'] == 'circle':
        r = float(item['r'])
        cx = _clamp(float(item['cx']) + dx, wall + r, float(width) - wall - r)
        cy = _clamp(float(item['cy']) + dy, wall + r, float(height) - wall - r)
        item['cx'] = float(cx)
        item['cy'] = float(cy)
        ...
        dx_lo = wall - min_x
        dx_hi = float(width) - wall - max_x
        dy_lo = wall - min_y
        dy_hi = float(height) - wall - max_y

        dx = _clamp(dx, dx_lo, dx_hi)
        dy = _clamp(dy, dy_lo, dy_hi)
        item['x1'] = float(x1 + dx)
        item['y1'] = float(y1 + dy)
        item['x2'] = float(x2 + dx)
        item['y2'] = float(y2 + dy)
...
def _randomize_obstacles(layout_obstacles, seed=None):
    if not _obstacle_randomization_enabled or _jitter_px <= 0:
        return [dict(obs) for obs in layout_obstacles]
    if seed is not None:
        rng = random.Random(int(seed))
    elif _jitter_seed is not None:
        rng = random.Random(int(_jitter_seed))
    else:
        rng = random.Random()
    return [_jitter_obstacle(obs, rng, _jitter_px) for obs in layout_obstacles]


def _get_density_spec(density_level):
    if density_level not in ObstacleDensity.ALL_LEVELS:
        raise ValueError(f"Invalid density level: {density_level}")
    return dict(OBSTACLE_DENSITY_SPECS[density_level])


def describe_obstacle_density(density_level=None):
    level = current_obstacle_density if density_level is None else density_level
    spec = _get_density_spec(level)
    return {
        'density_level': level,
        'count_scale': float(spec['count_scale']),
        'jitter_px': None if spec['jitter_px'] is None else int(spec['jitter_px']),
        'randomization_enabled': bool(spec['randomization_enabled']),
...
def regenerate_obstacles(count=None, seed=None, density_level=None, target_pos=None, map_layout=None):
    ...
    obstacles[:] = [dict(obs) for obs in WALL_OBSTACLES]
    if current_obstacle_density == ObstacleDensity.NONE:
        interior_obstacles = []
    else:
        interior_obstacles = OBSTACLE_PRESETS.get(
            current_obstacle_density,
            MAP_LAYOUT_PRESETS.get(current_map_layout, MAP_LAYOUT_PRESETS[MapLayout.DEFAULT]),
        )

    layout_obstacles = [dict(obs) for obs in interior_obstacles]
    layout_obstacles.extend([dict(obs) for obs in extra_obstacles])
    obstacles.extend(_randomize_obstacles(layout_obstacles, seed=seed))
...
def set_obstacle_density(density_level):
    ...
    if density_level not in ObstacleDensity.ALL_LEVELS:
        raise ValueError(f"Invalid density level: {density_level}")
    spec = describe_obstacle_density(density_level)
    set_obstacle_randomization(
        enabled=bool(spec['randomization_enabled']),
...
def set_obstacle_jitter(jitter_px=0, seed=0):
    ...
    _jitter_seed = None if seed is None else int(seed)


def set_obstacle_randomization(enabled=True, jitter_px=None, seed=None):
    """Global switch for randomized obstacle perturbation."""
    global _obstacle_randomization_enabled
    _obstacle_randomization_enabled = bool(enabled)
    if jitter_px is not None:
        set_obstacle_jitter(jitter_px=jitter_px, seed=seed)
    elif seed is not None:
        set_obstacle_jitter(jitter_px=_jitter_px, seed=seed)
