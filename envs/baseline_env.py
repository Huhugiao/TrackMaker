...
class BaselineEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        # Recreate the rule attacker after env.reset so optional per-episode
        # speed randomization in TADEnv is reflected in AttackerGlobalPolicy.
        if self.attacker_strategy_mode.lower() == "random":
            self._init_attacker_policy("random")
        elif self.attacker_strategy_mode.lower() != "static":
