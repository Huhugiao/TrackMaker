...
def _draw_trail_academic(surface, traj, color_rgb, width_px, time_markers=True, show_arrows=True):
...
def _draw_agent_academic(surface, agent, color_rgb, role=None, label=None, show_heading=True):
...
def _resolve_fov_angle_deg(fov_angle=None):
    """Return the active FOV angle without assuming a module-level setting."""
    if fov_angle is not None:
        return float(fov_angle)
    env_parameters = getattr(map_config, 'EnvParameters', None)
    return float(getattr(
        map_config,
        'FOV_ANGLE',
        getattr(env_parameters, 'FOV_ANGLE', 360.0),
    ))


def _prepare_fov_polygon_points(fov_points, fov_angle=None):
    """Remove the sector center from a full-circle FOV polygon.

    ``TrackingEnv`` stores FOV points as ``[center, boundary...]``.  That is
    correct for a partial sector, but for a 360-degree FOV it creates a radial
    closing seam from the defender to the range boundary.  A full-circle FOV
    is already described by its boundary points alone.
    """
    points = list(fov_points or [])
    if _resolve_fov_angle_deg(fov_angle) >= 359.0 and len(points) > 1:
        return points[1:]
    return points


def _draw_fov_academic(surface, tracker, fov_points=None, fov_angle=None):
    """Draw FOV cone with subtle fill."""
    if pygame is None or not fov_points or len(fov_points) < 3:
        return
    resolved_angle = _resolve_fov_angle_deg(fov_angle)
    polygon_points = _prepare_fov_polygon_points(fov_points, resolved_angle)
    if len(polygon_points) < 3:
        return
    # A 360-degree visibility range is contextual rather than directional, so
    # keep it quieter than a partial FOV cone.
    fill_alpha = 10 if resolved_angle >= 359.0 else 20
    fill_color = (100, 160, 255, fill_alpha)
    pygame.gfxdraw.filled_polygon(surface, polygon_points, fill_color)
    if resolved_angle >= 359.0:
        return

    # Partial FOV edge lines.
    center = fov_points[0]
    p_left = fov_points[1]
...
def get_canvas_tad(target, defender, attacker, defender_traj, attacker_traj,
                   surface=None, fov_points=None, collision_info=None,
                   fov_angle=None):
    ...
    _draw_fov_academic(surface, defender, fov_points, fov_angle=fov_angle)
    _draw_capture_sector_academic(surface, defender)
    _draw_capture_radius_academic(surface, defender)
    # Defender trail (blue)
    _draw_trail_academic(surface, defender_traj, (50, 100, 220), map_config.trail_width, show_arrows=False)
    # Attacker trail (red)
    _draw_trail_academic(surface, attacker_traj, (220, 80, 60), map_config.trail_width, show_arrows=False)
    # Draw agents
    if collision_info and collision_info.get('collision'):
        _draw_agent_academic(surface, defender, (220, 50, 50), role='defender', show_heading=False)
    else:
        _draw_agent_academic(surface, defender, (50, 100, 220), role='defender', show_heading=False)
    if collision_info and collision_info.get('attacker_collision'):
        _draw_agent_academic(surface, attacker, (180, 50, 220), role='attacker', show_heading=False)
    else:
        _draw_agent_academic(surface, attacker, (220, 80, 60), role='attacker', show_heading=False)
    # Target
...
class _MplRenderer:
    ...
    def render_frame(self, target, defender, attacker,
        ...
        if fov_points and len(fov_points) >= 3:
            ...
            fov_angle = _resolve_fov_angle_deg(kwargs.get('fov_angle'))
            polygon_world = _prepare_fov_polygon_points(fov_world, fov_angle)
            fov_poly = mpatches.Polygon(
                polygon_world, closed=True,
                facecolor=c_def,
                alpha=0.025 if fov_angle >= 359.0 else 0.06,
                edgecolor='none', zorder=1)
            ax.add_patch(fov_poly)
            # Only a partial FOV has meaningful radial boundaries.
            if fov_angle < 359.0:
                center = fov_world[0]
                p_left = fov_world[1]
                p_right = fov_world[-1]
        ...
        rgba = np.asarray(self.fig.canvas.buffer_rgba(), dtype=np.uint8)
        return rgba.reshape((nrows, ncols, 4))[..., :3].copy()
...
def get_canvas_tad_matplotlib(target, defender, attacker,
    ...
    try:
        ...
        return get_canvas_tad(target, defender, attacker,
                              ...
                              fov_angle=kwargs.get('fov_angle'))
