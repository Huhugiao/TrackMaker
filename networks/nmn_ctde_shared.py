"""Neural modular network with CTDE critic and shared radar encoder."""
...
class DefenderNetNMNCTDEShared(_NMNStageMixin, nn.Module):
    """NMN actor with CTDE critic and a shared radar encoder across actor/critic."""
    ...
    def _encode_radar(self, radar):
        return self.shared_radar_encoder(self._prepare_nmn_radar(radar))
    ...
    def _encode_actor_obs(self, obs):
        ...
        radar_emb = self._encode_radar(radar)
    ...
    def _encode_critic_obs(self, obs):
        ...
        defender_part = torch.cat([defender_scalar, self._encode_radar(defender_radar)], dim=-1)
        ...
        attacker_part = torch.cat([attacker_scalar, self._encode_radar(attacker_radar)], dim=-1)
    ...
    def forward(self, actor_obs, critic_obs):
        ...
        critic_state = self.critic_backbone(self._encode_critic_obs(critic_obs))
...
__all__ = ["DefenderNetNMNCTDEShared"]
