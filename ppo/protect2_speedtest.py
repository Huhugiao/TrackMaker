...
- 采样阶段内部：环境计算、网络推理、对手策略、GAE等
- 主循环阶段：权重同步、采样等待、数据拼接、网络更新
- 训练设备传输：D2H(权重拉回CPU) 与 H2D(训练批次拷贝到训练设备)

示例：
    python -m ppo.profile_protect2_speed --updates 30 --warmup-updates 3
"""
...
def _build_batch(rollouts: List[Dict], training_mode: str) -> Dict[str, np.ndarray]:
    if training_mode == 'rl':
        return {
    ...
    if training_mode == 'mixed':
        return {
            ...
            'returns': np.concatenate([r['returns'] for r in rollouts], axis=0),
            'values': np.concatenate([r['values'] for r in rollouts], axis=0),
            'expert_actions': np.concatenate([r['expert_actions'] for r in rollouts], axis=0),
        }
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Protect2训练耗时剖析')
    parser.add_argument('--skill-mode', type=str, default='protect2', help='技能模式，默认protect2')
    parser.add_argument('--training-mode', type=str, default='rl', choices=['rl', 'mixed'], help='训练模式')
    ...
    parser.add_argument('--network-type', type=str, default='nmn', help='网络类型')
    ...
    SetupParameters.SKILL_MODE = args.skill_mode
    ...
    ray.init(**ray_kwargs)
    try:
        model = Model(device=device, global_model=True, network_type=args.network_type)
        runners = [Runner.remote(i, network_type=args.network_type) for i in range(n_envs)]
        ...
        for update_idx in range(1, args.updates + 1):
            ...
            t0 = time.perf_counter()
            rollout_futures = [r.run.remote(n_steps, profile=True) for r in runners]
            rollouts = ray.get(rollout_futures)
            phase['rollout_collect'] = time.perf_counter() - t0

            rollout_timing_sum = defaultdict(float)
            for r in rollouts:
                _sum_numeric_dict(rollout_timing_sum, r.get('timings', {}))

            t0 = time.perf_counter()
            batch_cpu = _build_batch(rollouts, training_mode=args.training_mode)
            ...
            _sync_cuda(model.device)
            t0 = time.perf_counter()
            _train_update(model, batch_device, training_mode=args.training_mode)
            _sync_cuda(model.device)
        ...
        print(_format_table(
            headers=['阶段', '累计秒', '每env_step毫秒', '占总训练比例'],
        ...
        print(_format_table(
            headers=['采样子阶段', '累计秒', '每env_step毫秒', '占采样比例', '估算占总训练比例'],
            rows=_runner_rows(runner_totals, driver_totals),
        ))

        print('\n' + '=' * 72)
        print('4) 总体吞吐')
        print(f'统计update数: {counted_updates}')
        print(f'总env steps: {total_steps}')
        print(f'总墙钟时间: {wall_time:.3f}s')
        print(f'吞吐: {steps_per_sec:.2f} steps/s')
        print(f'平均: {ms_per_step:.4f} ms/env_step')
        print('=' * 72)
